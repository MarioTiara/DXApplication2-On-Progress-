SELECT 
		...
		WHEN (C.ORDER_NUMBER IS NOT NULL) AND ((C.STATUS = 'ON DELIVERY' AND (A.LAST_UPDATE < C.DUE_DATE)) OR C.STATUS = 'ARRIVE') THEN 'OTGUENSOND' 
		WHEN (C.ORDER_NUMBER IS NOT NULL) AND (C.STATUS = 'OVERNIGHT') THEN 'OTGUENSOVN' 
		WHEN (C.ORDER_NUMBER IS NOT NULL) AND ((((C.STATUS = 'KOSONGAN') OR (C.STATUS = 'WAITING')) AND (A.LAST_UPDATE > C.PICKUP_DATE)) OR ((C.STATUS = 'ON DELIVERY') AND (A.LAST_UPDATE > C.DUE_DATE))) THEN 'OTGUENSABN' 
		ELSE 'OFGOENSN' 
		END AS ORDSTS, 
...
LEFT JOIN CUSTOMER_PROJECT E ON E.PROJECT_SID = D.PROJECT_SID LEFT JOIN VehicleConfig F ON F.VehicleSid = A.VEHICLE_SID WHERE A.IS_ACTIVE = 1