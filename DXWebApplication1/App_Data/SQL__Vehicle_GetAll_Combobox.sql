SELECT 
	...
	C.SWITCH_FROM, 
	CASE 
		WHEN (C.ORDER_NUMBER IS NULL) AND (DATEDIFF(HOUR, A.LAST_UPDATE, GETDATE()) > 24) THEN 'OFGOENSN' 
		...
		WHEN (C.ORDER_NUMBER IS NOT NULL) AND ((((C.STATUS = 'Kosongan') OR (C.STATUS = 'Waiting')) AND (A.LAST_UPDATE > C.PICKUP_DATE)) OR ((C.STATUS = 'OnDelivery') AND (A.LAST_UPDATE > C.DUE_DATE))) THEN 'OTGUENSABN' ELSE 'OFGOENSN' 
	END AS ORDSTS, 
	CASE 
		WHEN DATEDIFF(HOUR, A.LAST_UPDATE, GETDATE()) > 24 THEN 'INACTIVE' 
	...
	LEFT JOIN CUSTOMER_PROJECT E ON E.PROJECT_SID = D.PROJECT_SID WHERE A.IS_ACTIVE = 1